...
- wrapper event coordination: pause preview capture and other expensive work while a thread is switching/loading
...
Implement the workaround as an CDP wrapper renderer patch or equivalent standalone app-process patch:
...
## CDP Wrapper Renderer Patch Plan

The CDP wrapper launches Codex Desktop with a local debugging port and injects renderer JavaScript directly. It does not depend on any external plugin runtime.

Use `renderer/fast-thread-loader.js` as the standalone renderer patch. `scripts/codex-perf-launch.py` injects it by default and requires no extra user clicks.
...
- pause expensive preview capture or diagnostics
...
The renderer patch should not make the user click "load older messages."
...
If the exact app-server request path is safely interceptable, defer repeated `thread/turns/list` calls for older pages during the initial switch. If the request path is not stable, fall back to DOM/render containment and wrapper event coordination.
...
The actual selectors must be verified against the current Codex DOM. The renderer patch should only apply rules to confirmed transcript/turn containers and should disable itself if selectors become ambiguous.
...
Injected code and diagnostics can listen and pause expensive work. For the installed tab switcher, the plan is:
...
- log performance summaries to wrapper metrics
- never mutate `~/.codex/state_5.sqlite`
- never mutate rollout JSONL

State repair remains a separate backup-first maintenance operation, not renderer behavior.

### Guardrails

The renderer patch must:
...
Acceptance for the interim renderer patch:
...
- no SQLite/JSONL mutation from the renderer patch
- disabling the renderer patch fully restores stock behavior
...
### CDP wrapper Renderer Patch

The CDP wrapper UX renderer patch is accepted only when all of these pass:

- It is injected by default and requires no user click to improve loading.
- It emits performance marks for navigation start, first paint, and navigation end.
- It coordinates through wrapper events so tab preview capture and similar expensive work pause during thread navigation.
...
8. Implement or package the CDP wrapper thread-loading UX renderer patch.
